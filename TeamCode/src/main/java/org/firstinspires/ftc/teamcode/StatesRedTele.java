...
@Config
@TeleOp(name="Houston Red Teleop")
public class StatesRedTele extends LinearOpMode{
    ...
    public static double T_SHOOT = 0.25;
    //time in seconds for ball injection
...
//        RevColorSensorV3 lastColor = hardwareMap.get(RevColorSensorV3.class, "bruhColor");
       ...
       while(opModeIsActive()){
           ...
           double turretPos =  0.59 - (theta*degreeToServo) - 0.1 + addOn - 0.005;
           ...
           if(turrUpCurr && !turrUpLast){
               autoAim = false;
               rotOne.setPosition(rotOne.getPosition() - 0.0025);
               rotTwo.setPosition(rotTwo.getPosition() - 0.0025);
               counter --;
           }
